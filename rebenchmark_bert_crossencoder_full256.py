...
def encode_frame(frame: pd.DataFrame, tokenizer: BertTokenizer, max_length: int) -> EncodedPairDataset:
    enc = tokenizer(
    ...
    labels = torch.tensor(frame["is_duplicate"].astype(int).to_numpy(), dtype=torch.long)
    return EncodedPairDataset(enc, labels)
...
def make_row(model_name: str, model: nn.Module, val_loader: DataLoader, test_loader: DataLoader, threshold: float, size_path: Path, scope: str, train_n: int, val_n: int, test_n: int) -> dict[str, object]:
    ...
    return {
        ...
        "Embedding Source": "bert-base-uncased",
        ...
        "Actual Epochs Run": None,
        "Source File": "outputs/results/bert_crossencoder_quantized_results_full.csv",
        "Validation Accuracy": float(accuracy_score(val_y, (val_prob >= threshold).astype(int))),
        "Validation F1": float(f1_score(val_y, (val_prob >= threshold).astype(int), zero_division=0)),
        "Model Path": str(size_path),
...
def main() -> None:
    ...
    val_loader = DataLoader(encode_frame(val_df, tokenizer, MAX_LENGTH), batch_size=BATCH_SIZE, shuffle=False)
    test_loader = DataLoader(encode_frame(test_df, tokenizer, MAX_LENGTH), batch_size=BATCH_SIZE, shuffle=False)
    ...
    float_size_path = MODEL_DIR / "bert_crossencoder_float_full.pt"
    ...
    int8_size_path = MODEL_DIR / "bert_crossencoder_int8_full.pt"
    torch.save(int8_model.state_dict(), int8_size_path)

    rows = [
        make_row("BERT Cross-Encoder", float_model, val_loader, test_loader, threshold, float_size_path, "full_test", len(train_df), len(val_df), len(test_df)),
        make_row("BERT Cross-Encoder Int8", int8_model, val_loader, test_loader, threshold, int8_size_path, "full_test", len(train_df), len(val_df), len(test_df)),
    ]

    RESULT_DIR.mkdir(parents=True, exist_ok=True)
    out_path = RESULT_DIR / "bert_crossencoder_quantized_results_full.csv"
